        ...
        if serializer.is_valid():
            ...
            if user.id:
                with connection.cursor() as c:
                    c.execute(
                        """select is_hod,email from ci_erp_users where id = %s""", [user.id]
                    )
...
class CiDesignationViewSet(APIView):
    ...
    def get(self, request, *args, **kwargs):
        with connection.cursor() as cursor:
            cursor.execute("""
                ...
                JOIN ci_departments dept ON d.department_id = dept.department_id
            """)
    ...
    def patch(self, request, pk, *args, **kwargs):
        ...
        for key in ["designation_code", "company_id", "designation_name", "description", "created_at", "department_id"]:
            if key in data:
        ...
        with connection.cursor() as cursor:
            ...
            cursor.execute("""
                SELECT 
                    ...
                    CONCAT(u.first_name, ' ', u.last_name) AS employee_name,
                    u.id AS user_id,
                    (CHAR_LENGTH(alloc.policy_id) - CHAR_LENGTH(REPLACE(alloc.policy_id, ',', '')) + 1) AS total_policies,
                    SUM(CASE WHEN pa.acknowledge = 'Y' THEN 1 ELSE 0 END) AS acknowledged_count,
                    MAX(pa.created_date) AS last_acknowledge_date
    ...
    def get(self, request):
        try:
            with connection.cursor() as cursor:
                 ...
                 # Basic counts
                cursor.execute("""SELECT COUNT(*) FROM ci_erp_users""")
        ...
        if action == "return_yes":
            try:
                with connection.cursor() as cursor:
                    ...
                    if asset_data:
                        assets_name, serial_number, company_asset_code, employee_name, employee_email, employee_id ,quantity = asset_data
    ...
    def get(self, request):
        with connection.cursor() as cursor:
            cursor.execute("""
                ...
                JOIN ci_policies p ON FIND_IN_SET(p.policy_id, pa.policy_id) > 0
    ...
    def get(self, request, emp_id):
        try:
            with connection.cursor() as cursor:
                ...
                cursor.execute("""
                    SELECT LENGTH(policy_id) - LENGTH(REPLACE(policy_id, ',', '')) + 1 AS policy_count
                    FROM ci_policy_allocations
                    WHERE emp_id = %s;
                """, [emp_id])
