...
class EmployeeAttendance(APIView):
    ...
    def _get_work_timings(self, office_shift_id, employee_hub_name, attendance_date):
        ...
        day_name = attendance_date.strftime(
            "%A"
        ).lower()  # Get day name (monday, tuesday, etc.)
        ...
        with connection.cursor() as cursor:
            ...
            if shift_row and shift_row[0] and shift_row[1]:
                try:
                    ...
                    logger.warning(
                        f"Error parsing shift times for employee_hub {employee_hub_name}: {e}"
                    )
    ...
    def _handle_clock_in(
        ...
        with connection.cursor() as cursor:
            ...
            if cursor.fetchone()[0] > 0:
                ...
                return Response(
                    {"status": "Clock-in recorded in secondary log table"},
                    status=status.HTTP_200_OK,
                )

        with connection.cursor() as c:
            c.execute(
                """select office_shift_id from ci_erp_users_details where employee_id = %s""",
                [emp_id],
            )

            row = c.fetchone()
            office_shift_id = row[0] if row and row[0] is not None else 9
        ...
        in_time, _ = self._get_work_timings(
            office_shift_id, employee_hub_name, attendance_date
        )
        ...
        if late_mark == "Y":
            late_mark_count = self._get_current_month_late_marks(
                emp_id, attendance_date
            )
        else:
            late_mark_count = 0
...
class DashboardAttendance(APIView):
    ...
    def get(self, request, employee_id):
        ...
        try:
            with connection.cursor() as c:
                c.execute(
                    """select clock_in, clock_out from ci_biomatric_data_2 where login_date = %s and emp_id = %s order by id desc limit 1""",
...
class AssetsInventory(APIView):
    ...
    def get(self, request):
        ...
        with connection.cursor() as c:
            ...
            c.execute(
                """
                SELECT asi.category_id, cat.category_name, asi.brand_id, brd.category_name as brand_name, asi.product_id, prd.category_name as product_name, SUM(quantity) AS total_purchased
                FROM ci_assets_instock asi left join ci_erp_constants cat on asi.category_id = cat.constants_id
                    left join ci_erp_constants brd on asi.brand_id = brd.constants_id
                    left join ci_erp_constants prd on asi.product_id = prd.constants_id
            ...
            for row in purchased_rows:
                category_id, category_name, brand_id, brand_name, product_id, product_name, total_purchased = row
        ...
        for key, val in purchased_map.items():
            ...
            currently_assigned = assigned - returned
 
            # in_stock = total - assigned + returned
            in_stock = total - currently_assigned
...
class ViewNotification(APIView):
    ...
    def get(self, request):
        ...
        try:
            with connection.cursor() as c:
                ...
                c.execute("""select * from ci_notification where send_to_id = %s and is_click = 'N'""", [user_id])
    ...
    def patch(self, request):
        ...
        if not all([notification_id, action]):
            return Response({"status":"error","message": "notification_id or action is missing"}, status=status.HTTP_400_BAD_REQUEST)
       
        try:
            with connection.cursor() as c:
 
                if action == 'view':
                    c.execute("""update ci_notification set is_view = 'Y' where notification_id = %s""", [notification_id])
                elif action == 'click':
                    c.execute("""update ci_notification set is_click = 'Y' where notification_id = %s""", [notification_id])
 
            return Response({"status":"success","message": f"Notification updated successfully for {action} action"}, status=status.HTTP_200_OK)
