                ...
                response = [
                    {
                        ...
                        "profile_photo": (
                            request.build_absolute_uri(
                                f"/hrms-backend/media/documents/{quote(row[11])}"
                            )
                            if row[11]
                            else None
                        ),
        ...
        try:
            with connection.cursor() as c:
                ...
                c.execute(
                    """
                    SELECT 
                        DATE_FORMAT(attendance_date, '%%W') AS day,
                        DATE_FORMAT(attendance_date, '%%d-%%m-%%Y') AS date,
                        CASE 
    ...
    def get(self, request):
        ...
        try:
            with connection.cursor() as c:
                ...
                c.execute(
                    """select holiday_id,event_name,CASE WHEN ec.category_name IS NOT NULL THEN ec.category_name ELSE h.state END AS state,eh.employee_hub_name,start_date,end_date,is_publish as status from ci_holidays h left join ci_erp_constants ec on h.state = ec.constants_id left join ci_employee_hub eh on h.employee_hub = eh.employee_hub_id;"""
                )
    ...
    def post(self, request):
        ...
        try:
            with connection.cursor() as c:
                ...
                c.execute(
                    """select employee_name, month, year, pr.created_at as salary_payment_date, pr.employee_id, pr.employee_name, department_name, ud.date_of_joining, designation_name, u.city as location, payable_days, ud.bank_name, ud.account_number as bank_account_number, pf_number, esic_number, pan_number, basic_plus_da, hra, medical_allowance, conveyance_allowance, arrears, total_earnings, pf, esic, pt, tds, mlwf, tds, other_deduction, total_deduction, net_pay from ci_payroll_report pr inner join ci_erp_users_details ud on pr.employee_id = ud.employee_id inner join ci_erp_users u on ud.user_id = u.id inner join ci_departments dpt on ud.department_id = dpt.department_id inner join ci_designations dsg on pr.designation_id = dsg.designation_id where pr.payroll_report_id = %s""",
                    [payroll_report_id],
                )
...
#                         INSERT INTO ci_erp_users 
#                         (first_name, middle_name, last_name, email, contact_number, is_active, gender, username, password, 
#                          user_role_id, user_type, company_id, state, employee_hub_id, created_at)
#                         VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, NOW())
#                         """,
#                         [
#                             first_name, middle_name, last_name, email, phone,
#                             user_status, gender, username, hashed_password,
#                             user_type, user_type, company_id, state_id, employee_hub_id,
#                         ],
#                     )
#                     user_id = c.lastrowid
